...
/**
 * Servlet implementation class UpdateSeasonController
 */
@WebServlet("/updateSeason.ad")
public class UpdateSeasonController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// post 방식 인코딩 
				...
				String infoSeason = multiRequest.getParameter("infoSeason");
				String infoDeleteYn = multiRequest.getParameter("infoDeleteYn");
				int cpage = Integer.parseInt(multiRequest.getParameter("cpage"));
				
				switch(infoSeason) {
				case "봄" : infoSeason = "spring";
				break;
				case "여름" : infoSeason = "summer";
				break;
				case "가을" : infoSeason = "fall";
				break;
				case "겨울" : infoSeason = "winter";
				}
				// 가공
...
//						System.out.println(at.getFilePath());
					...
					int result = new InfoMenuService().updateSeasonInfo(info, at, infoNo);
					
					if(result > 0) {
						// 1) 성공했으니 다시 리스트로 들어가고 alertMsg 보내주자
						request.getSession().setAttribute("alertMsg", "게시글 수정 완료");
						response.sendRedirect(request.getContextPath() + "/season.ad?cpage=" + cpage + "&deleteYn=" + infoDeleteYn + "&season=" + infoSeason);
					} else {
