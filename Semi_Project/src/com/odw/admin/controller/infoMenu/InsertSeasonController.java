...
/**
 * Servlet implementation class InsertSeasonController
 */
@WebServlet("/insertSeason.ad")
public class InsertSeasonController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public InsertSeasonController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			int result = new InfoMenuService().insertSeasonInfo(info, at);
			
			// 5) 화면지정
			if(result > 0) {
				// 성공했으니 다시 리스트로 들어가고 alertMsg를 보내주자
				request.getSession().setAttribute("alertMsg", "정보글 등록 완료");
				response.sendRedirect(request.getContextPath() + "/season.ad?cpage=1");
			} else {
