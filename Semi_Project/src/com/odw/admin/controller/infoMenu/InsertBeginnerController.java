...
/**
 * Servlet implementation class InsertBeginnerController
 */
@WebServlet("/insertBeginner.ad")
public class InsertBeginnerController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public InsertBeginnerController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			int result = new InfoMenuService().insertBeginnerInfo(info, at);
			
			// 5) 화면지정
			if(result > 0) {
				// 성공했으니 다시 리스트로 들어가고 alertMsg를 보내주자
				request.getSession().setAttribute("alertMsg", "정보글 등록 완료");
				response.sendRedirect(request.getContextPath() + "/beginner.ad?cpage=1");
			} else {
