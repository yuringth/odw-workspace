...
/**
 * Servlet implementation class UpdateLocalMountainController
 */
@WebServlet("/updateLocal.ad")
public class UpdateLocalMountainController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UpdateLocalMountainController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			infoLocal = multiRequest.getParameter("infoLocal");
			
			if(infoLocal.equals("서울")) {
				infoLocal = "S";
			} else if(infoLocal.equals("인천")) {
				infoLocal = "I";
			}
			...
			int result = new InfoMenuService().updateLocalInfo(info, at, infoNo);
...
//				System.out.println(deleteYn);
				...
				response.sendRedirect(request.getContextPath() + "/local.ad?cpage=" + cpage + "&deleteYn=" + deleteYn + "&local=" + infoLocal);
			} else {
				// 실패했을 때다
